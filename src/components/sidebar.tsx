        ...
        <div className="glass-card-strong border-t border-white/10 rounded-none flex items-stretch justify-around px-1 py-1">
          {mobileTabItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center justify-center gap-0.5 flex-1 py-2 min-h-[52px] rounded-lg transition-colors ${
              ...
              >
                ...
                {isActive && (
                  <div className="w-4 h-0.5 rounded-full gradient-blue-purple mt-0.5" />
                )}
              </Link>
            )
          })}
          {hasMore && (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-0.5 flex-1 py-2 min-h-[52px] rounded-lg transition-colors ${
                moreOpen ? 'text-white' : 'text-white/40'
              }`}
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
